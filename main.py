...
class BaselineExperiment:
    ...
    def run_baseline_only(self, epochs=30, dataset='bloodmnist'):
        """只运行主实验的baseline"""
        print(f"Starting baseline experiment on {dataset.upper()}, device: {self.device}")
        ...
        # 只运行baseline实验
        try:
            self.run_baseline_experiment(epochs)
        except Exception as e:
...
def main():
    ...
    for dataset in datasets:
        ...
        # 打印当前数据集结果
        if 'original_baseline_bilstm' in results:
            baseline_result = results['original_baseline_bilstm']
            final_results = baseline_result['final_results']
            
            print(f"\n{'='*60}")
            print(f"{dataset.upper()} BASELINE EXPERIMENT RESULTS")
            print(f"{'='*60}")
            print(f"Model: {baseline_result['config']['name']}")
            print(f"Final AUC: {final_results['auc']:.4f}")
            print(f"Final Accuracy: {final_results['accuracy']:.4f}")
            print(f"{'='*60}")
        else:
            print(f"{dataset.upper()} baseline experiment failed to complete.")
    ...
    for dataset in datasets:
        if dataset in all_results and 'original_baseline_bilstm' in all_results[dataset]:
            result = all_results[dataset]['original_baseline_bilstm']['final_results']
            print(f"📊 {dataset.upper():12} - AUC: {result['auc']:.4f} | Accuracy: {result['accuracy']:.4f}")
        else:
            print(f"❌ {dataset.upper():12} - Failed to complete")

    print(f"\n🎉 Multi-dataset baseline experiments completed!")
    print("✅ All three datasets have been evaluated")
